...
user.getDepartmentName = function() {
    try {
        ...
        if (user.getCache('is_leader'))
            ...
            return 'Руководство';
        return enums.fractionListId[user.getCache('fraction_id')].departmentList[user.getCache('rank_type1')];
    }
    catch (e) {
        methods.debug(e);
    }
...
user.getRankName = function() {
    try {
        ...
        return enums.fractionListId[user.getCache('fraction_id')].rankList[user.getCache('rank_type1')][user.getCache('rank1')];
    }
    catch (e) {
        methods.debug(e);
    }
...
user.isDepLeader = function() {
    return user.isLogin() && user.getCache('fraction_id') > 0 && user.getCache('rank1') === 0;
};

user.isDepSubLeader = function() {
    return user.isLogin() && user.getCache('fraction_id') > 0 && user.getCache('rank1') === 1;
};
