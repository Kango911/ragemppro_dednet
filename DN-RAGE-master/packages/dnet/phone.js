...
phone.memberAction = function(player, id) {
    ...
    mysql.executeQuery(`SELECT id, social, name, fraction_id, rank1, rank_type1, is_sub_leader FROM users WHERE id = '${methods.parseInt(id)}'`, (err, rows, fields) => {
        ...
        rows.forEach(row => {
            ...
            if (!row['is_sub_leader']) {
                ...
                let rankList = [];
                fractionItem.rankList[row['rank_type1']].forEach((item, id) => {
                    if (id == row['rank1'])
                ...
                items.push(phone.getMenuItemRadio(
                    'Изменить должность',
                    'Текущая должность: ' + fractionItem.rankList[row['rank_type1']][row['rank1']],
                    'Выберите должность',
                    rankList,
                    { name: 'none' },
                    '',
                ...
                if (user.isLeader(player) || user.isSubLeader(player)) {
                    ...
                    items.push(phone.getMenuItemRadio(
                        ...
                        'Текущий отдел: ' + fractionItem.departmentList[row['rank_type1']],
                        'Выберите отдел',
                        depList,
                        { name: 'none' },
                        '',
...
phone.fractionList = function(player) {
    ...
    mysql.executeQuery(`SELECT id, social, name, fraction_id, rank1, rank_type1, is_leader, is_sub_leader, is_online, login_date FROM users WHERE fraction_id = '${fractionId}' ORDER BY is_leader ASC, is_sub_leader ASC, rank_type1 ASC, is_online DESC, rank1 ASC, name ASC`, (err, rows, fields) => {
        let items = [];
        let depName = '';
        let depList = [];
        let depPrev = -1;
        ...
        let rankType = user.get(player, 'rank_type1');
        let canEdit = user.get(player, 'rank1') == 0 || user.get(player, 'rank1') == 1;
        ...
        rows.forEach(row => {
            ...
            if (row['is_leader']) {
                ...
                if (depPrev != row['rank_type1']) {
                    if (depList.length > 0)
                        items.push(phone.getMenuMainItem(`${depName} | ${depList.length} чел.`, depList));
                    depName = fractionItem.departmentList[row['rank_type1']];
                    depList = [];
                }

                if (isLeader || isSubLeader || (canEdit && rankType == row['rank_type1'])) {
                    depList.push(phone.getMenuItemUser(
                        row['name'],
                        fractionItem.rankList[row['rank_type1']][row['rank1']] + desc,
                        row['is_online'] === 1,
                        { name: 'memberAction', memberId: row['id'] },
                        'https://a.rsg.sc//n/' + row['social'].toLowerCase(),
                        true,
...
phone.fractionVehicles = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE fraction_id = '${fractionId}' AND is_buy = '1' ORDER BY rank_type1 ASC, rank1 DESC, name ASC`, (err, rows, fields) => {
        rows.forEach(veh => {
            if (depPrev != veh['rank_type1']) {
                if (depList.length > 0)
                    items.push(phone.getMenuMainItem(`${depName} | ${depList.length} шт.`, depList));
                depName = fractionItem.departmentList[veh['rank_type1']];
                depList = [];
            }

            let rankName = fractionItem.rankList[veh['rank_type1']][veh['rank1']];

            if (isDepLeader && veh['rank_type1'] == rankType || canEdit) {
...
phone.fractionVehicles2 = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE fraction_id = '${fractionId * -1}' AND is_buy = '1' ORDER BY rank_type1 ASC, rank1 DESC, name ASC`, (err, rows, fields) => {
        rows.forEach(veh => {
            if (depPrev != veh['rank_type1']) {
                if (depList.length > 0)
